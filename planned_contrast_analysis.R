...
contrasts(performance_data1$TRT) <- contr.notemp
attributes(performance_data1$TRT)

#step 4 - run the linear model with planned contrasts
m1 <- lm(LIVE_WEIGHT_STARTER ~ TRT, data = performance_data1)
summary(m1)
...
m2 <- lm(LIVE_WEIGHT_GROWER ~ TRT, data = performance_data1)
summary(m2)
...
m3 <- lm(LIVE_WEIGHT_TOTAL ~ TRT, data = performance_data1)
summary(m3)
...
m4 <- lm(FI_STARTER.PEN ~ TRT, data = performance_data1)
summary(m4)
...
m5 <- lm(FI_GROWER.PEN ~ TRT, data = performance_data1)
summary(m5)
...
m6 <- lm(FI_TOTAL.PEN ~ TRT, data = performance_data1)
summary(m6)
...
m7 <- lm(FCR_STARTER.PEN ~ TRT, data = performance_data1)
summary(m7)
...
m8 <- lm(FCR_GROWER.PEN ~ TRT, data = performance_data1)
summary(m8)
